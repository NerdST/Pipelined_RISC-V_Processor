...
## Time Taken
This lab took 6-8 hours to do. A lot of the time I spent was debugging various parts of the design, connecting different wires to different modules took some time.

Another portion that took a good amount of time was creating a testbench program that didn't have hazards. I found a tool online that converted assembly to machine code which significantly helped with this process:
- https://github.com/Kritagya-Agarwal/Assembly-To-Machine-Code-RISC-V
- https://luplab.gitlab.io/rvcodecjs/
  - Another tool I used for verifying individual lines instead of compiling an entire assembly program

## Figure 1 Markup
![RISC-V Pipelined Diagram with LUI](riscv-pipeline-markup.png)

## LUI Supported Tables
...
<!-- ## RISC-V Diagram
![RISC-V Multicycle Diagram](lab8-diagram.png) -->
...
#       Instruction             Description             Address         Machine Code
        ...
        addi x11, x0,  12       # x11 = 12              0x4             0x00C00593
        addi x0,  x0,  0        # nop                   0x8             0x00000013
        addi x0,  x0,  0        # nop                   0xC             0x00000013
        addi x0,  x0,  0        # nop                   0x10            0x00000013
        or   x12, x11, x10      # x12 = 17              0x14            0x00A5E633
        and  x13, x11, x10      # x13 = 4               0x18            0x00A5F6B3
        sw   x10, 12(x0)        # Mem[12] = 5           0x1C            0x00A02623
        beq  x10, x0, skip      # not taken             0x20            0x00050663
        addi x11, x11, 1        # x11 = 13               0x24            0x00158593
        beq x0, x0, taken       # taken                 0x28            0x00000463

skip:   addi x3, x0, 10         # shouldn't happen      0x2C            0x00A00193
taken:  lw   x13, 12(x0)        # x13 = Mem[12]         0x30            0x00C02683
        addi x0,  x0,  0        # nop                   0x34            0x00000013
        addi x0,  x0,  0        # nop                   0x38            0x00000013
        addi x0,  x0,  0        # nop                   0x3C            0x00000013

        lui x4, 20              # x4 = 20 * 2^12        0x40            0x00014237
        # gets forwarded
        addi x5, x4, 15         # x5 = x4 + 15          0x44            0x00F20293
        # x10 = 5 = 0x0101, x11 = 13 = 0x1101
        and x1, x10, x11        # x1 = x10 & x11        0x48            0x00B570B3
        or x2, x10, x11         # x2 = x10 | x11        0x4C            0x00B56133
        add x3 , x10, x11       # x3 = x10 + x11        0x50            0x00B501B3
        sub x4, x10, x11        # x4 = x10 - x11        0x54            0x40B50233
        andi x5, x10, 255       # x5 = 5                0x58            0x0FF07293
        ori x1, x0, 255         # x1 = 255              0x5C            0x0FF06093
        slti x2, x10, 100       # x2 = 1                0x60            0x06452113
        slt x3, x10, x11        # x3 = 0                0x64            0x00B521B3
        addi x0,  x0,  0        # nop                   0x68            0x00000013
        addi x0,  x0,  0        # nop                   0x6C            0x00000013
        sw   x0,  100(x0)       # Mem[100] = 0          0x70            0x06002223
...
![Waveform Image](lab9-waveforms.png)
This contains additional waves that I used for debugging, but the required waves are located at the top.

## TA Lab Checkoff
![TA Lab Checkoff](lab9-checkoff.png)

## RTL Viewer Schematic
Condensed RTL view of the processor
![Condensed RTL Viewer Schematic](lab9-rtl-condensed.png)
Expanded, much less readable
![Expanded RTL Viewer Schematic](lab9-rtl-expanded.png)
